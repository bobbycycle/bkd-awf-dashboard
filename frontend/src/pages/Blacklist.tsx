...
export default function Blacklist() {
  ...
  useEffect(() => {
    api.blacklist.list().then(l => setList(l as IpEntry[]))
  }, [])

  async function add() {
    if (!newIp.trim()) return
    const entry = await api.blacklist.add({ ip: newIp.trim(), reason: newReason || 'Manual block', expires_at: null }) as IpEntry
    setList(prev => [entry, ...prev])
    setNewIp(''); setNewReason('')
  }

  async function remove(ip: string) {
    await api.blacklist.remove(ip)
  ...
  return (
    <div className="p-6">
      <div className="flex items-center gap-2 mb-5">
        <ShieldOff size={16} className="text-[#f85149]" />
        <div>
          <h1 className="text-white font-semibold text-[15px]">Blacklist</h1>
          <p className="text-[#8b949e] text-[11px] mt-0.5">{list.length} blocked IPs</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-4">
        ...
        <button onClick={add}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-[#f85149] hover:bg-[#da3633] text-white rounded text-[12px] font-semibold transition-colors">
          <Plus size={12} /> Block
        </button>
      </div>
      ...
      <div className="bg-[#161b22] border border-[#2a3348] rounded-lg overflow-hidden">
        <table className="w-full text-[12px]">
          <thead>
            <tr className="text-[#8b949e] border-b border-[#2a3348] bg-[#0f1117]">
              ...
              <th className="text-left px-4 py-2.5 font-medium w-[140px]">Expires</th>
            ...
            {list.map(e => (
              <tr key={e.ip} className="border-b border-[#21293a] hover:bg-[#1c2230]">
                ...
                <td className="px-4 py-2.5 text-[#8b949e]">
                  {e.expires_at ? fmt(e.expires_at) : <span className="text-[#4d5a6b]">permanent</span>}
                </td>
                <td className="px-4 py-2.5 text-right text-[#f85149] font-mono tabular-nums">{e.hits}</td>
                <td className="px-4 py-2.5 text-right">
                  <button onClick={() => remove(e.ip)}
        ...
        {list.length === 0 && (
          <div className="flex items-center justify-center h-24 text-[#4d5a6b] text-[12px]">Blacklist is empty</div>
        )}
      </div>
    </div>
  )
